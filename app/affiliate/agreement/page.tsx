...
import { BadgeCheck, Clock3, Download, FileSignature, ShieldCheck } from 'lucide-react';
...
export default async function Page() {
  ...
  return <Shell>
    <section className="glass rounded-[2rem] p-7 md:p-10">
      <div className="flex flex-wrap items-start justify-between gap-5"><div><p className="eyebrow">Electronic agreement</p><h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">Your partner terms</h1><p className="mt-3 max-w-3xl text-slate-300">Review the exact commercial terms before signing. The signed snapshot is hashed and retained as immutable evidence.</p></div><div className="flex flex-col items-end gap-3"><span className="badge"><ShieldCheck aria-hidden size={15} />Verified portal identity</span>{currentAgreement ? <Link href="/affiliate/agreement/print" className="btn btn-muted min-h-10 px-4 text-sm"><Download aria-hidden size={15} />Download PDF</Link> : null}</div></div>
